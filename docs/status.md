...
- Local server check:
  ...
  - Confirmed the human-approved `Ș`/`Ț` convention: exact bucket IDs, word files, route segments, and image folders; ASCII word ID and image filename prefixes `ro-sh-...` and `ro-tz-...`.
  ...
  - Verified Batch 8 with `bun run validate:content`, `bun run lint`, `./node_modules/.bin/tsc --noEmit --incremental false`, `bun src/game/gameplay-compatibility.check.ts`, `bun src/content/matching.check.ts`, and `git diff --check`; `pnpm` was unavailable and `http://localhost:3000` was not reachable for browser verification.
...
Admin Words Inventory, Locale-Wide Content Refactor, Gameplay Inclusion Mode Selector, and Romanian Content Expansion Batch 8 are implementation-complete and accepted with documented local verification exceptions. Browser verification for `/ro`, affected play routes including `/ro/play/ș`, `/ro/play/ț`, `/ro/play/f`, `/ro/play/d`, and `/admin/words` remains pending after the user starts the dev server on port `3000`. There is no Batch 9 currently defined for Romanian Content Expansion; the next product decision is whether to open a new content scope.
...
- Placeholder-backed approved words require explicit human approval or a documented hiding plan; Batch 4 uses draft status to keep new image-less words out of child-facing gameplay.
...
- `Ș` and `Ț` starting-letter buckets use exact bucket IDs, word files, route segments, and image folders, with ASCII word ID and image filename prefixes `ro-sh-...` and `ro-tz-...`.
- Future `Ă`, `Â`, and `Î` starting-letter word ID tokens are not resolved yet and should be decided before production manifests are added for those buckets.
