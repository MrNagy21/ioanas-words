...
Phase: Batch 8 implementation complete; admin words inventory Batch 4 complete; admin release review route complete with Release 1/Release 2/Release 3 split; locale-wide content refactor complete; gameplay inclusion mode selector and large-pool setup selection complete; wheel setup UX redesign implementation complete; Romanian content expansion Batch 108 Slice 8 QA complete after `ramă`/`râmă` promotion; Wheel Setup Exclusion Filters saved setup editing UX overhaul complete; Wheel Visible Subset Reset Bug implementation and browser QA complete; Batch 5 partially complete

Overall status: Static Romanian starter content drives an interactive SVG wheel with segment labels, ready image rendering, stable placeholder fallbacks, spin selection, and spin animation. The wheel itself is the primary spin target, decorative frame effects stay fixed outside the rotating surface, word labels and the center letter stay upright, and spin animation uses a longer physical deceleration. Batch 4 adds the result modal, close/keep flow, client-side word removal, reset current letter, choose-another-letter paths, reduced-motion spin behavior, and a clear empty-wheel state after all current-letter words are removed. The user-reported visible subset reset bug is fixed and verified: `15`-word wheels backed by larger eligible pools now count down after repeated `Scoate din roată` actions without hidden off-wheel words automatically refilling removed slots. Batch 5 added local static content validation for Romanian letters, word manifests, image paths, duplicate IDs, letter buckets, and production status rules. Batch 6 selected pixel art, AI batch generation with human review, `256 x 256 px` lossless WebP, size targets, prompt/style guidelines, and word-ID naming conventions in `docs/image-pipeline.md`, then added local ready-image validation and a dry-run capable optimizer command. Locale-Wide Content Refactor, Gameplay Inclusion Mode Selector, Wheel Setup UX Redesign, Wheel Setup Exclusion Filters, saved setup editing UX, production UX audit follow-up, Wheel Visible Subset Reset Bug work, and the read-only `/admin/releases` release review area are complete. Romanian Content Expansion Batch 108 completed Slice 8 QA after Batch 107 promoted all `15` Slice 8 rows, including the human-approved `ramă` / `râmă` pair with distinct IDs and a narrow validator allowlist. A follow-up image-style audit regenerated all `15` Slice 8 assets as pixel-art replacements after the first assets were found too smooth/CGI-like. Current production content is `599` total Romanian records, `596` approved words, `596` ready images, `0` approved placeholders, and `3` rejected audit-trail rows. Release 2 remains the committed frozen content release with `109` IDs, and Release 3 now lists the `120` current dirty-tree additions for QA.

## Completed

- Completed Romanian Content Expansion Batch 108 Slice 8 coverage QA and next-prompt preparation on 2026-06-29.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-108.md` with exact Batch 107 promotion checks, identity QA, image QA, current totals, `/admin/releases/3` count, verification results, localhost route QA, and the Batch 109 planning prompt.
- Completed Romanian Content Expansion Batch 107 Slice 8 production promotion on 2026-06-29 after the human explicitly approved promoting both `ramă` and `râmă`.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-107.md` and `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-106.md`.
- Promoted all `15` Slice 8 rows into `content/ro/words-l.json`, `content/ro/words-r.json`, and `content/ro/words-s.json`: `leopard`, `lână`, `lan`, `lampion`, `ramă`, `râmă`, `rândunică`, `rechin`, `riglă`, `rulotă`, `ruladă`, `radio`, `romb`, `stea`, and `saltea`.
...
- Added a narrow validator allowlist for the exact Romanian `ramă` / `râmă` duplicate-normalized pair while preserving duplicate-normalized errors for all other cases.
...
- Updated `src/game/gameplay-compatibility.check.ts` because `CHI` mixed coverage is now `17`.
...
- Completed a follow-up Release 3 image-style audit on all `120` dirty-tree image records and regenerated the `15` Slice 8 assets as pixel-art replacements with circular-safe padding.
- Completed Romanian Content Expansion Batch 105 Slice 8 production planning on 2026-06-29.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-105.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, image brief direction, route/validation impact, coverage deltas, active rows, blocked rows, and the required `ramă`/`râmă` decision.
...
- Completed Romanian Content Expansion Batch 104 Slice 7 coverage QA and next-prompt preparation on 2026-06-29.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-104.md` with exact Batch 103 promotion checks, image QA, current totals, affected coverage, `/admin/releases/2` count, verification targets, localhost route targets, and the Batch 105 planning prompt.
- Completed Romanian Content Expansion Batch 103 Slice 7 production promotion on 2026-06-29 after the human waived the separate image-approval stop for this orchestrator run and asked the orchestrator to proceed while preserving image QA and circular-mask safety.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-103.md` with image-processing notes, final asset sizes, production content changes, totals, coverage deltas, fixture updates, and verification handoff.
...
- Completed Romanian Content Expansion Batch 102 Slice 7 image briefs/contact sheet on 2026-06-29.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-102.md` with scope guard, row-major contact-sheet mapping, human waiver note for the separate image approval stop, per-word image decisions, circular-mask corrections, image metadata, and QA requirements.
- Completed Romanian Content Expansion Batch 101 Slice 7 production planning on 2026-06-29.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-101.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, image brief direction, route/validation impact, coverage deltas, preserved decisions, and the Batch 102 image-generation/promotion/QA prompt.
...
- Confirmed no exact word/display collisions, normalized collisions, planned ID collisions, or planned image-path collisions against current production content for the Slice 7 candidates.
...
- Completed Romanian Content Expansion Batch 100 Slice 6 coverage QA and next-prompt preparation on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-100.md` with exact Batch 99 promotion checks, Batch 99 asset dimensions and byte sizes, circular-mask review notes, current totals, affected coverage, `/admin/releases/2` count, verification targets, localhost route targets, and the Batch 101 planning prompt.
- Completed Romanian Content Expansion Batch 99 Slice 6 production promotion on 2026-06-28 after the human waived the separate image-approval stop for this orchestrator run and asked the orchestrator to proceed while preserving image QA and circular-mask safety.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-99.md` with image-processing notes, final asset sizes, production content changes, totals, coverage deltas, fixture updates, and verification handoff.
...
- Completed Romanian Content Expansion Batch 98 Slice 6 image briefs/contact sheet on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-98.md` with scope guard, row-major contact-sheet mapping, human waiver note for the separate image approval stop, per-word image decisions, circular-mask corrections, image metadata, and QA requirements.
- Completed Romanian Content Expansion Batch 97 Slice 6 production planning on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-97.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, image brief direction, route/validation impact, coverage deltas, preserved decisions, and the Batch 98 image-generation/promotion/QA prompt.
...
- Confirmed no exact word/display collisions, normalized collisions, planned ID collisions, or planned image-path collisions against current production content for the Slice 6 candidates.
...
- Recorded that if all Slice 6 rows later pass image review and promotion, direct starts-with deltas would be `C +4`, `P +1`, `Ș +8`, `T +1`, and `B +1`.
...
- Completed Romanian Content Expansion Batch 96 Slice 5 coverage QA and next-prompt preparation on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-96.md` with exact Batch 95 promotion checks, Batch 95 asset dimensions and byte sizes, circular-mask review notes, current totals, affected coverage, `/admin/releases/2` count, verification targets, localhost route targets, and the Batch 97 planning prompt.
- Completed Romanian Content Expansion Batch 95 Slice 5 production promotion on 2026-06-28 after the human waived the separate image-approval stop for this orchestrator run and asked the orchestrator to proceed while preserving image QA and circular-mask safety.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-95.md` with image-processing notes, final asset sizes, production content changes, totals, coverage deltas, fixture updates, and verification handoff.
...
- Completed Romanian Content Expansion Batch 94 Slice 5 image briefs/contact sheet on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-94.md` with scope guard, row-major contact-sheet mapping, human waiver note for the separate image approval stop, per-word image decisions, circular-mask corrections, image metadata, and QA requirements.
- Completed Romanian Content Expansion Batch 93 Slice 5 production planning on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-93.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, image brief direction, route/validation impact, coverage deltas, preserved decisions, and the Batch 94 image-generation/promotion/QA prompt.
...
- Confirmed no exact word/display collisions, normalized collisions, planned ID collisions, or planned image-path collisions against current production content for the Slice 5 candidates.
...
- Completed Romanian Content Expansion Batch 92 Slice 4 coverage QA and next-prompt preparation on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-92.md` with exact Batch 91 promotion checks, Batch 91 asset dimensions and byte sizes, circular-mask review notes, current totals, affected coverage, `/admin/releases/2` count, verification results, localhost status, and the Batch 93 planning prompt.
- Completed Romanian Content Expansion Batch 91 Slice 4 production promotion on 2026-06-28 after the human waived the separate image-approval stop for this orchestrator run and asked the orchestrator to proceed while preserving image QA and circular-mask safety.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-91.md` with image-processing notes, circular-mask QA, final asset sizes, production content changes, totals, coverage deltas, verification, localhost status, and representative attempted image URLs.
...
- Completed Romanian Content Expansion Batch 90 Slice 4 image briefs/contact sheet on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-90.md` with scope guard, row-major contact-sheet mapping, human waiver note for the separate image approval stop, brand consistency notes, circular-mask constraints, per-word briefs, generation prompt/metadata, and QA requirements.
- Created the ordered, unlabeled review contact sheet at `assets/image-sources/ro/batch-90/contact-sheet.png` for `disc`, `diamant`, `dungă`, `dronă`, `valiză`, `vrabie`, `vulcan`, `val`, `vâslă`, `velier`, `violetă`, `vultur`, `ventuză`, `evantai`, and `navă`.
- Completed Romanian Content Expansion Batch 89 Slice 4 production planning on 2026-06-28.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-89.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, image brief direction, route/validation impact, coverage deltas, preserved decisions, and the Batch 90 image-brief/contact-sheet prompt.
...
- Confirmed no exact word/display collisions, normalized collisions, planned ID collisions, or planned image-path collisions against current production content for the Slice 4 candidates.
...
- Recorded that if all Slice 4 rows later pass image review and promotion, direct starts-with deltas would be `D +4`, `V +9`, `E +1`, and `N +1`.
...
- Set Romanian Content Expansion Batch 90 as the next image-brief/contact-sheet batch for the active Slice 4 candidates, with a hard stop before production promotion.
- Completed Romanian Content Expansion Batch 88 Slice 3 coverage QA and next-prompt preparation on 2026-06-26.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-88.md` with exact Batch 87 promotion checks, image dimensions and byte sizes, circular-mask review notes, current totals, affected coverage, `/admin/releases/2` count, full verification results, localhost status, and the Batch 89 planning prompt.
- Confirmed Batch 87 promoted exactly the `15` human-approved Slice 3 rows, all Batch 87 images are canonical `256 x 256 px` WebP assets below `20 KB` and below the `12 KB` warning threshold, and the optimized circular preview remains acceptable for `jet`, `ambalaj`, `etaj`, `peisaj`, `majoretă`, `pajiște`, `coji`, `drapel`, and `dalmațian`.
- Confirmed current production totals remain `524` total Romanian records, `521` approved words, `521` ready images, `0` approved placeholders, and `3` rejected audit-trail records.
- Confirmed affected coverage remains `J` `16` starts / `19` contains-only / `35` mixed and `D` `21` / `30` / `51`; `/admin/releases/2` includes all `15` Batch 87 IDs and has `154` dirty-tree additions.
...
- Set Romanian Content Expansion Batch 89 as the next planning-only production-slice batch for Slice 4: `disc`, `diamant`, `dungă`, `dronă`, `valiză`, `vrabie`, `vulcan`, `val`, `vâslă`, `velier`, `violetă`, `vultur`, `ventuză`, `evantai`, and `navă`.
- Completed Romanian Content Expansion Batch 87 Slice 3 production promotion on 2026-06-26 after the human accepted all Batch 86 images with "Images are approved, please go on."
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-87.md` with image-processing notes, circular-mask QA, final asset sizes, production content changes, totals, coverage deltas, verification, localhost status, and the Batch 88 prompt.
...
- Added Batch 86 image-processing artifacts under `assets/image-sources/ro/batch-86/`, including `contact-sheet.png`, `final-sources/`, `work-128/`, `final-preview.png`, `circular-preview.png`, `optimized-preview.png`, and `optimized-circular-preview.png`.
- Updated `/admin/releases/2` to include all `15` Batch 87 IDs; Release 2 now lists `154` dirty-tree additions for QA.
...
- Completed Romanian Content Expansion Batch 86 Slice 3 image briefs/contact sheet on 2026-06-24.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-86.md` with scope guard, row-major contact-sheet mapping, human review notes, brand consistency review, per-word briefs, generation prompt/metadata, and a human-validation gate.
- Created the ordered, unlabeled review contact sheet at `assets/image-sources/ro/batch-86/contact-sheet.png` for `jet`, `ambalaj`, `etaj`, `bujor`, `peisaj`, `majoretă`, `prăjitor`, `pajiște`, `coji`, `drum`, `dulceață`, `dop`, `dar`, `drapel`, and `dalmațian`.
...
- Completed Romanian Content Expansion Batch 85 Slice 3 production planning on 2026-06-24.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-85.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, route/validation impact, promotion risks, target coverage impact, active image-brief list, and the Batch 86 image-brief/contact-sheet prompt.
...
- Set Romanian Content Expansion Batch 86 as the next image-brief/contact-sheet batch for the active Slice 3 candidates, with a hard stop before production promotion.
- Completed Romanian Content Expansion Batch 84 Slice 2 coverage QA and next-prompt preparation on 2026-06-24.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-84.md` with exact Batch 83 promotion checks, image dimensions and sizes, circular-mask review notes, current totals, affected coverage, `/admin/releases/2` count, full verification results, localhost route QA, and the Batch 85 planning prompt.
- Confirmed Batch 83 promoted exactly the `15` human-approved Slice 2 rows, all Batch 83 images are canonical `256 x 256 px` WebP assets below `20 KB` and below the `12 KB` warning threshold, and the optimized circular preview remains acceptable for `ham`, `jambon`, `ținută`, `țesătură`, and `țâșnitoare`.
- Confirmed current production totals remain `509` total Romanian records, `506` approved words, `506` ready images, `0` approved placeholders, and `3` rejected audit-trail records.
- Confirmed affected coverage remains `H` `24` starts / `64` contains-only / `88` mixed, `J` `15` / `11` / `26`, `Ț` `14` / `44` / `58`, `S` `25` / `38` / `63`, `Ș` `18` / `31` / `49`, `T` `18` / `115` / `133`, `R` `22` / `187` / `209`, `D` `15` / `30` / `45`, and `L` `22` / `104` / `126`.
- Confirmed `/admin/releases/2` includes all `15` Batch 83 IDs and now has `139` dirty-tree additions.
...
- Set Romanian Content Expansion Batch 85 as the next planning-only production-slice batch for Slice 3: `jet`, `ambalaj`, `etaj`, `bujor`, `peisaj`, `majoretă`, `prăjitor`, `pajiște`, `coji`, `drum`, `dulceață`, `dop`, `dar`, `drapel`, and `dalmațian`.
- Completed Romanian Content Expansion Batch 83 Slice 2 production promotion on 2026-06-24 after the human accepted all Batch 82 images with "images are fine."
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-83.md` with image-processing notes, circular-mask QA, final asset sizes, production content changes, totals, coverage deltas, verification, localhost route QA, and the Batch 84 prompt.
- Promoted all `15` accepted Batch 82 cells into `content/ro/words-h.json`, `content/ro/words-j.json`, and `content/ro/words-ț.json`: `hamsie`, `hienă`, `homar`, `ham`, `hanorac`, `hidrant`, `jurnal`, `jalon`, `jambon`, `joystick`, `jerseu`, `țintă`, `ținută`, `țesătură`, and `țâșnitoare`.
- Added final `256 x 256 px` lossless WebP assets under `public/images/ro/h/`, `public/images/ro/j/`, and `public/images/ro/ț/`; every Batch 83 asset is below the `12 KB` warning threshold and the `20 KB` hard maximum.
...
- Sandboxed localhost checks failed, then required outside-sandbox retries returned `200 OK` for `/ro`, `/admin/words`, `/admin/releases/2`, `/ro/play/h`, `/ro/play/j`, `/ro/play/tz`, `/images/ro/h/ro-h-ham.webp`, `/images/ro/j/ro-j-joystick.webp`, and `/images/ro/ț/ro-tz-tasnitoare.webp`.
- Completed Romanian Content Expansion Batch 82 Slice 2 image briefs/contact sheet on 2026-06-24.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-82.md` with scope guard, row-major contact-sheet mapping, human review notes, brand consistency review, per-word briefs, generation prompt/metadata, and a Batch 83 prompt gated on explicit human image acceptance.
- Created the ordered, unlabeled review contact sheet at `assets/image-sources/ro/batch-82/contact-sheet.png` for `hamsie`, `hienă`, `homar`, `ham`, `hanorac`, `hidrant`, `jurnal`, `jalon`, `jambon`, `joystick`, `jerseu`, `țintă`, `ținută`, `țesătură`, and `țâșnitoare`.
...
- Completed Romanian Content Expansion Batch 81 Slice 2 production planning on 2026-06-24.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-81.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, image brief direction, route/validation impact, localhost follow-up status, and the Batch 82 image-brief/contact-sheet prompt.
...
- Set Romanian Content Expansion Batch 82 as the next image-brief/contact-sheet batch for the active Slice 2 candidates.
- Completed the deferred Batch 80 localhost follow-up after the user started the expected dev server: sandboxed localhost still failed to connect, but outside-sandbox checks returned `200 OK` for `/ro`, `/admin/words`, `/admin/releases/2`, `/ro/play/h`, and representative Batch 79 images for `horn`, `hol`, `horă`, `hochei`, and `hering`.
- Completed Romanian Content Expansion Batch 80 Slice 1 coverage QA and next-prompt preparation on 2026-06-24.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-80.md` with exact Batch 79 promotion checks, image dimensions and sizes, circular-mask review notes, current totals, affected coverage, `/admin/releases/2` count, full verification results, localhost status, and the Batch 81 planning prompt.
- Confirmed Batch 79 promoted exactly the `15` human-approved `H` rows, all Batch 79 images are canonical `256 x 256 px` WebP assets below `20 KB` and below the `12 KB` warning threshold, and the optimized circular preview remains acceptable for `horn`, `hol`, `horă`, `hochei`, and `hering`.
- Confirmed current production totals remain `494` total Romanian records, `491` approved words, `491` ready images, `0` approved placeholders, and `3` rejected audit-trail records.
- Confirmed affected coverage remains `H` `18` starts / `64` contains-only / `82` mixed, `GE` `9` / `8` / `17`, and `CHE` `2` / `15` / `17`.
...
- Completed Romanian Content Expansion Batch 79 Slice 1 production promotion on 2026-06-23.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-79.md` with human acceptance, image-processing notes, circular-mask QA, final asset sizes, production content changes, totals, affected coverage, verification, localhost status, and the Batch 80 prompt.
...
- Added final `256 x 256 px` lossless WebP assets under `public/images/ro/h/`; every Batch 79 asset is below the `12 KB` warning threshold and the `20 KB` hard maximum.
...
- Completed Romanian Content Expansion Batch 78 Slice 1 image briefs/contact sheet on 2026-06-23.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-78.md` with scope guard, row-major contact-sheet mapping, human review notes, per-word briefs, generation prompt/metadata, and a Batch 79 prompt gated on explicit human image acceptance.
- Created the ordered, unlabeled review contact sheet at `assets/image-sources/ro/batch-78/contact-sheet.png` for `hipopotam`, `hârtie`, `hotel`, `halat`, `harpă`, `horn`, `hamster`, `hambar`, `husă`, `hol`, `horă`, `hochei`, `hublou`, `hamburger`, and `hering`.
...
- Completed Romanian Content Expansion Batch 77 Slice 1 planning on 2026-06-23.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-77.md` with source/spelling checks, duplicate/collision checks, canonical file/ID/path plans, metadata plans, speech-target notes, image brief direction, route/validation impact, and the Batch 78 image-brief/contact-sheet prompt.
...
- Completed Romanian Content Expansion Batch 76 orchestrator scope split on 2026-06-23.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-plan-index-batch-76.md` with the accepted-pool dedupe audit, current-production collision check, exact cross-listed words, the `ramă` / `râmă` normalized/ID blocker, `11` bounded slices, key per-slice risks, verification notes, and the Batch 77 planning-only prompt.
...
- Prepared Romanian Content Expansion Batch 76+ implementation orchestrator prompt on 2026-06-23.
- Added `docs/app-development-program/features/romanian-content-expansion/implementation-orchestrator-prompt-batch-76.md` with the human acceptance decision, exact non-hold accepted candidate pool, excluded hold list, subagent workflow, image-validation stop gate, promotion verification checklist, browser checks, documentation requirements, and final closeout criteria.
- Completed Romanian Content Expansion Batch 75 high-value target top-up worksheet on 2026-06-23.
- Added `docs/app-development-program/features/romanian-content-expansion/high-value-target-top-up-worksheet-batch-75.md` with current coverage, proposed candidate rows for the requested letters and sequence groups, image notes for unclear words, review rules, source basis, and the Batch 76 planning prompt.
...
- Completed Romanian Content Expansion Batch 74 final requirement top-up on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-74.md` with source checks, image-processing notes, asset sizes, content totals, point 2 coverage, Release 2 updates, verification, browser QA, and the release QA prompt.
...
- Completed Romanian Content Expansion Batch 73 production promotion on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-73.md` with human review decisions, image-processing notes, asset sizes, content totals, coverage, verification, localhost route QA, and the Batch 74 prompt.
...
- Added Romanian Content Expansion Batch 72 point 2 sequence top-up review sheet on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-72.md` with DEX/dexonline source checks, candidate rationale, row-major mapping, human feedback, corrective generation prompt, and Batch 73 promotion prompt.
- Generated and copied the unlabeled four-cell review sheet to `assets/image-sources/ro/batch-72/contact-sheet.png` for `ghebe`, `ghioc`, `pereche`, and `rechizite`.
- Recorded human feedback: `rechizite` is accepted, `pereche` is accepted as a word but needs a clearer pair image, and `ghebe` / `ghioc` should be changed.
- Generated and copied the unlabeled corrective review sheet to `assets/image-sources/ro/batch-72/corrective-contact-sheet-1.png` for `gheretă`, `unghieră`, and a clearer socks-based `pereche` image.
...
- Added creator attribution on 2026-06-13: shared `CreatorCredit` link to Ioana Nagy appears in the locale app shell for all supported locale routes and in the footer of all current `/admin` pages, with matching root metadata author/creator fields.
- Updated the read-only `/admin/releases` area on 2026-06-13 to support `/admin/releases/1` and `/admin/releases/2`, splitting review state into `Release 1` committed baseline (`367` approved words) and `Release 2` dirty-tree additions (`104` newly added approved words), with shared word detail dialogs and affected wheel links.
- Completed Romanian Content Expansion Batch 71 slice coverage QA and next-prompt preparation on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-71.md` with canonical totals, Batch 70 asset QA, requested-target coverage, verification results, preserved decisions, and the Batch 72 planning prompt.
...
- Verification passed on 2026-06-13: `pnpm run validate:content`, `pnpm run lint`, `pnpm exec tsc --noEmit --incremental false`, `pnpm run check:gameplay`, `pnpm run check:matching`, `pnpm run check:setup-storage`, `pnpm run build`, and `git diff --check`.
...
- Redirected Romanian Content Expansion Batch 72 from broad planning to the focused point 2 sequence top-up review after the release-readiness check found no new Release 2 `GHE`, `GHI`, `CHE`, or `CHI` rows.
- Completed Romanian Content Expansion Batch 70 production promotion on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-70.md` with human review decisions, removed `răchită`, promoted rows, image QA artifacts, final asset sizes, content totals, requested-target coverage, verification, localhost route QA, and the Batch 71 prompt.
...
- Used the first corrective sheet option for `rășină` and the first corrective sheet option for `gingie`; `răchită` was not promoted.
...
- Set Romanian Content Expansion Batch 71 as the next slice coverage QA and next-prompt preparation batch.
- Completed Romanian Content Expansion Batch 69 corrective image pass on 2026-06-13.
...
- Completed Romanian Content Expansion Batch 69 image briefs and contact sheet on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-69.md` with per-word briefs for the original `15` review cells and revised it after human feedback to keep `14` active promotion candidates.
- Generated one ordered, unlabeled Batch 69 review contact sheet at `assets/image-sources/ro/batch-69/contact-sheet.png`.
- Generated one Batch 69 style comparison sheet at `assets/image-sources/ro/batch-69/comparison-sheet.png`.
...
- Recorded the ready-to-copy Batch 70 prompt for promoting only human-accepted cells and corrective variants after review.
- Completed Romanian Content Expansion Batch 68 production-slice planning on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-68.md` with source checks, canonical file/ID/path plans, metadata plans, route/loader/validation impact, coverage deltas, preserved decisions, and the Batch 69 prompt.
...
- Set Romanian Content Expansion Batch 69 as the next image-brief/contact-sheet batch for human review.
- Completed Romanian Content Expansion Batch 67 slice coverage QA and next-prompt preparation on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-67.md` with canonical totals, Batch 66 asset QA, requested-target coverage, verification results, preserved decisions, and the Batch 68 planning prompt.
...
- Set Romanian Content Expansion Batch 68 as the next planning-only production slice from the remaining already human-reviewed Batch 43 candidate pool.
- Completed Romanian Content Expansion Batch 66 production promotion on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-66.md` with human review decisions, the `tacâmuri` revision, promoted rows, image QA artifacts, final asset sizes, content totals, requested-target coverage, verification plan, and the Batch 67 prompt.
...
- Set Romanian Content Expansion Batch 67 as the next slice coverage QA and next-prompt preparation batch.
- Completed Romanian Content Expansion Batch 65 image briefs and contact sheet on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-65.md` with per-word briefs for `bibliotecă`, `radiator`, `sacou`, `șoim`, `ștampilă`, `șezlong`, `tacâmuri`, `țarc`, `țiglă`, `viespe`, `șurubelniță`, `joben`, `jantă`, `coțofană`, and `jardinieră`.
- Generated one ordered, unlabeled Batch 65 review contact sheet at `assets/image-sources/ro/batch-65/contact-sheet.png`.
- Generated one Batch 65 style comparison sheet at `assets/image-sources/ro/batch-65/comparison-sheet.png`.
...
- Recorded the ready-to-copy Batch 66 prompt for promoting only human-accepted cells after review.
- Completed Romanian Content Expansion Batch 64 production-slice planning on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-64.md` with source checks, canonical file/ID/path plans, metadata plans, route/loader/validation impact, coverage deltas, preserved decisions, and the Batch 65 prompt.
- Planned the next `15` rows: `bibliotecă`, `radiator`, `sacou`, `șoim`, `ștampilă`, `șezlong`, `tacâmuri`, `țarc`, `țiglă`, `viespe`, `șurubelniță`, `joben`, `jantă`, `coțofană`, and `jardinieră`.
...
- Recorded planned impact if all Batch 64 rows are later promoted with ready images: production totals would become `457` approved words and `457` ready images; requested-target deltas would be `Ă +7 contains`, `Â +1 contains`, `L +5 contains`, `R +1 starts/+4 contains`, `S +1 starts/+1 contains`, `Ș +4 starts`, `T +1 starts/+4 contains`, `Ț +2 starts/+2 contains`, `V +1 starts`, `B +1 starts/+2 contains`, `D +2 contains`, and `J +3 starts`.
- Recorded the ready-to-copy Batch 65 prompt for image briefs and one human-review contact sheet.
- Completed Romanian Content Expansion Batch 63 slice coverage QA and next-prompt preparation on 2026-06-13.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-63.md` with canonical totals, requested-target coverage, scope guard checks, Batch 62 asset dimensions and sizes, localhost route QA, verification, next-step decision, and the Batch 64 prompt.
- Confirmed the current Romanian pack has `442` approved words, `442` ready images, `0` approved placeholders, and `3` rejected audit-trail rows.
...
- Set Romanian Content Expansion Batch 64 as the next planning-only production slice.
- Completed Romanian Content Expansion Batch 62 production promotion on 2026-06-12.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-62.md` with human review decisions, promoted rows, image QA artifacts, final asset sizes, content totals, coverage impact, verification, localhost route QA, and the Batch 63 prompt.
...
- Sandboxed localhost checks failed, then required outside-sandbox retries returned `200` for `/ro`, `/admin/words`, affected play routes, and representative Batch 62 image URLs.
- Recorded the ready-to-copy Batch 63 prompt for slice coverage QA and next-prompt preparation.
- Completed Romanian Content Expansion Batch 61 image briefs and contact sheet on 2026-06-12.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-61.md` with per-word image briefs for `locomotivă`, `sac`, `soră`, `gălușcă`, `fundiță`, `punguță`, `covor`, `cravată`, `lavetă`, `porumbel`, `roabă`, `cabană`, `pădure`, `medalie`, and `bomboană`.
- Generated one ordered, unlabeled Batch 61 review contact sheet at `assets/image-sources/ro/batch-61/contact-sheet.png`.
...
- Completed Romanian Content Expansion Batch 60 production-slice planning on 2026-06-12.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-60.md` with source checks, canonical file/ID/path plans, metadata plans, route/loader/validation impact, coverage deltas, image brief direction, preserved decisions, and the Batch 61 prompt.
...
- Confirmed Batch 60 stayed planning-only with no production JSON records, image assets, placeholders, route/schema changes, speech-target production metadata, database/auth/billing/admin features, AI pronunciation, or clinical claims.
...
- Recorded planned impact if all Batch 60 rows are later promoted with ready images: production totals would become `442` approved words and `442` ready images; requested-target deltas would be `Ă +11 contains`, `L +2 starts/+3 contains`, `R +1 starts/+5 contains`, `S +2 starts`, `Ș +1 contains`, `T +3 contains`, `Ț +2 contains`, `V +4 contains`, `B +1 starts/+3 contains`, and `D +3 contains`.
- Recorded the ready-to-copy Batch 61 prompt for image briefs and one human-review contact sheet.
- Completed Romanian Content Expansion Batch 59 slice coverage QA and next-prompt preparation on 2026-06-12.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-59.md` with canonical totals, requested-target coverage, scope guard checks, Batch 58 asset dimensions and sizes, resolved localhost route QA, verification, and the Batch 60 prompt.
- Confirmed the current Romanian pack has `427` approved words, `427` ready images, `0` approved placeholders, and `3` rejected audit-trail rows.
...
- Set Romanian Content Expansion Batch 60 as the next planning-only production slice.
- Completed Romanian Content Expansion Batch 58 production promotion on 2026-06-12.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-58.md` with human approval, promoted rows, image QA artifacts, final asset sizes, content totals, verification, and the Batch 59 prompt.
- Promoted all `15` accepted Batch 57 cells: `biberon`, `borcan`, `burete`, `brad`, `bluză`, `bancă`, `dormitor`, `periuță`, `brățară`, `sticlă`, `șină`, `vierme`, `vișine`, `papuci`, and `lădiță`.
- Used the Batch 57 corrective sheet for `dormitor`, `brățară`, `sticlă`, and `vișine`.
- Confirmed all Batch 58 images are `256 x 256 px` lossless WebP files below the `12 KB` warning threshold and `20 KB` hard maximum.
...
- Updated the content-derived `CI` gameplay compatibility fixture because `papuci` increases the mixed pool from `19` to `20`.
- Verification passed on 2026-06-12: `pnpm run validate:content`, `pnpm run lint`, `pnpm exec tsc --noEmit --incremental false`, `pnpm run check:gameplay`, `pnpm run check:matching`, `pnpm run build`, and `git diff --check`.
...
- Completed Romanian Content Expansion Batch 57 image briefs and contact sheet on 2026-06-12.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-57.md` with per-word briefs for `biberon`, `borcan`, `burete`, `brad`, `bluză`, `bancă`, `dormitor`, `periuță`, `brățară`, `sticlă`, `șină`, `vierme`, `vișine`, `papuci`, and `lădiță`.
- Generated one ordered, unlabeled Batch 57 review contact sheet at `assets/image-sources/ro/batch-57/contact-sheet.png`.
...
- Recorded human feedback that `dormitor`, `brățară`, `sticlă`, and `vișine` needed correction or stronger disambiguation.
- Generated corrective contact sheet `assets/image-sources/ro/batch-57/corrective-contact-sheet-1.png` for `dormitor`, `brățară`, `sticlă`, and `vișine`.
- Recorded the `vișine` constraint: darker/smaller fruit plus a cut fruit can bias the image toward sour cherries, but no-label art cannot fully guarantee a young child will not read it as `cireșe`.
- Recorded the ready-to-copy Batch 58 prompt for human-accepted image promotion and full validation.
- Completed Romanian Content Expansion Batch 56 production-slice planning on 2026-06-12.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-56.md` with source checks, canonical row plans, coverage impact, preserved decisions, and the Batch 57 prompt.
- Planned the next `15` rows: `biberon`, `borcan`, `burete`, `brad`, `bluză`, `bancă`, `dormitor`, `periuță`, `brățară`, `sticlă`, `șină`, `vierme`, `vișine`, `papuci`, and `lădiță`.
- Confirmed Batch 56 stayed planning-only with no production JSON records, image assets, placeholders, route/schema changes, speech-target production metadata, database/auth/billing/admin features, AI pronunciation, or clinical claims.
- Recorded planned impact if all Batch 56 rows are later promoted with ready images: production totals would become `427` approved words and `427` ready images; requested-target deltas would be `Ă +7 contains`, `CI +1 contains`, `L +1 starts/+2 contains`, `R +8 contains`, `S +1 starts`, `Ș +1 starts/+1 contains`, `T +3 contains`, `Ț +3 contains`, `V +2 starts`, `B +7 starts`, and `D +1 starts/+2 contains`.
- Preserved the decisions that no `Ă` or `Â` starts-with rows are forced, no `Q`/`W`/`Y` padding is added, `cameră web` stays in `C`, `iac` stays in `I`, and `webcam` / `yak` stay out unless explicitly reopened.
- Completed Romanian Content Expansion Batch 55 slice coverage QA and next-prompt preparation on 2026-06-11.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-55.md` with canonical totals, requested-target coverage including `GHI`, scope guard checks, Batch 54 asset dimensions and sizes, localhost status, verification, and the Batch 56 prompt.
- Confirmed the current Romanian pack has `412` approved words, `412` ready images, `0` approved placeholders, and `3` rejected audit-trail rows.
...
- Local browser/app QA could not run because `http://localhost:3000` was unavailable both inside and outside the sandbox; no alternate dev server or port was started.
- Set Romanian Content Expansion Batch 56 as the next planning-only production slice.
- Completed Romanian Content Expansion Batch 54 production promotion on 2026-06-11.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-54.md` with human approval, promoted rows, image QA artifacts, final asset sizes, content totals, coverage impact, verification, and the Batch 55 prompt.
- Promoted all `15` accepted Batch 53 cells: `lebădă`, `lopată`, `lipici`, `lupă`, `leagăn`, `rac`, `ren`, `rucsac`, `role`, `supă`, `sare`, `semințe`, `sită`, `dovlecel`, and `țelină`.
...
- Updated content-derived gameplay compatibility fixtures because `dovlecel` increases the `V` contains-only and `CE` mixed pools, and `lipici` increases the `CI` mixed pool.
- Verification passed on 2026-06-11: `pnpm run validate:content`, `pnpm run lint`, `pnpm exec tsc --noEmit --incremental false`, `pnpm run check:gameplay`, `pnpm run check:matching`, `pnpm run build`, and `git diff --check`.
- Local browser/app QA could not run because `http://localhost:3000` was unavailable both inside and outside the sandbox; no alternate dev server or port was started.
- Completed Romanian Content Expansion Batch 53 image briefs and contact sheet on 2026-06-11.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-53.md` with per-word image briefs for `lebădă`, `lopată`, `lipici`, `lupă`, `leagăn`, `rac`, `ren`, `rucsac`, `role`, `supă`, `sare`, `semințe`, `sită`, `dovlecel`, and `țelină`.
- Generated one ordered, unlabeled Batch 53 review contact sheet at `assets/image-sources/ro/batch-53/contact-sheet.png`.
...
- Recorded human-review attention points for `lopată` circular crop comfort, `rac` lobster/crayfish reading, `sare` salt-versus-sugar readability, and `semințe` thumbnail readability.
- Recorded the ready-to-copy Batch 54 prompt for human-accepted image promotion and full validation.
- Completed Romanian Content Expansion Batch 52 production-slice planning on 2026-06-11.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-52.md` with source checks, canonical file/ID/path plans, metadata plans, coverage deltas, route/loader/validation impact, image brief direction, preserved decisions, and the Batch 53 prompt.
- Planned the Batch 52 third slice in order: `lebădă`, `lopată`, `lipici`, `lupă`, `leagăn`, `rac`, `ren`, `rucsac`, `role`, `supă`, `sare`, `semințe`, `sită`, `dovlecel`, and `țelină`.
- Confirmed Batch 52 stayed planning-only with no production JSON records, image assets, placeholders, route/schema changes, speech-target production metadata, database/auth/billing/admin features, AI pronunciation, or clinical claims.
- Recorded planned impact if all Batch 52 rows are later promoted with ready images: production totals would become `412` approved words and `412` ready images; requested-target deltas would be `Ă +7 contains`, `L +5 starts/+3 contains`, `R +4 starts/+1 contains`, `S +4 starts/+1 contains`, `D +1 starts/+1 contains`, `Ț +1 starts/+1 contains`, `T +2 contains`, `V +1 contains`, `B +1 contains`, `CE +1 contains`, and `CI +1 contains`.
- Preserved the decisions that no `Ă` or `Â` starts-with rows are forced, no `Q`/`W`/`Y` padding is added, `cameră web` stays in `C`, `iac` stays in `I`, and `webcam` / `yak` stay out unless explicitly reopened.
- Completed Romanian Content Expansion Batch 51 slice coverage QA and next-prompt preparation on 2026-06-11.
- Added `docs/app-development-program/features/romanian-content-expansion/slice-coverage-qa-batch-51.md` with totals, coverage, scope guard checks, asset checks, localhost status, verification, next-step decision, and the Batch 52 prompt.
- Confirmed the Batch 50 state remains `397` approved words, `397` ready images, `0` approved placeholders, and `3` rejected audit-trail rows.
- Confirmed all Batch 50 assets are `256 x 256 px`, below `20 KB`, and below the `12 KB` warning threshold.
- Confirmed Batch 52 was the planning-only next slice from the already human-reviewed Batch 43 candidate pool.
- Completed Romanian Content Expansion Batch 50 production promotion on 2026-06-11.
- Promoted all `15` accepted Batch 49/50 rows: `kimono`, `karate`, `kilt`, `yoga`, `tren`, `tractor`, `tablă`, `tigaie`, `tamburină`, `trandafir`, `ventilator`, `vopsea`, `volan`, `vestă`, and `vânătă`.
- Generated and promoted a white `kimono` replacement with a blue belt per human direction.
- Added `docs/app-development-program/features/romanian-content-expansion/production-promotion-batch-50.md` with human review decisions, circular-mask QA, brand comparison, final asset sizes, content totals, verification, and the Batch 51 prompt.
- Confirmed all Batch 50 images are `256 x 256 px` lossless WebP files below the `12 KB` warning threshold and `20 KB` hard maximum.
...
- Verification passed on 2026-06-11: `pnpm run validate:content`, `pnpm run lint`, `pnpm exec tsc --noEmit --incremental false`, `pnpm run check:gameplay`, `pnpm run check:matching`, `pnpm run build`, and `git diff --check`.
- Local browser/app QA could not run because `http://localhost:3000` was unavailable both inside and outside the sandbox; no alternate dev server or port was started.
- Completed Romanian Content Expansion Batch 49 image briefs and contact sheet on 2026-06-02.
- Added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-49.md` with per-word briefs for `kimono`, `karate`, `kilt`, `yoga`, `tren`, `tractor`, `tablă`, `tigaie`, `tamburină`, `trandafir`, `ventilator`, `vopsea`, `volan`, `vestă`, and `vânătă`.
- Generated one ordered, unlabeled Batch 49 review contact sheet at `assets/image-sources/ro/batch-49/contact-sheet.png`.
...
- Recorded the ready-to-copy Batch 50 prompt for human-accepted image promotion and full validation.
- Completed Romanian Content Expansion Batch 48 second production-slice planning on 2026-06-02.
- Added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-48.md` with source checks, canonical file/ID/path plans, metadata plans, coverage deltas, route/loader/validation impact, image brief direction, preserved decisions, and the Batch 49 prompt.
- Planned the Batch 48 second slice in order: `kimono`, `karate`, `kilt`, `yoga`, `tren`, `tractor`, `tablă`, `tigaie`, `tamburină`, `trandafir`, `ventilator`, `vopsea`, `volan`, `vestă`, and `vânătă`.
- Confirmed Batch 48 stayed planning-only with no production JSON records, image assets, placeholders, route/schema changes, speech-target production metadata, database/auth/billing/admin features, AI pronunciation, or clinical claims.
- Recorded planned impact if all Batch 48 rows are later promoted with ready images: production totals would become `397` approved words and `397` ready images; requested-target deltas would be `Ă +4 contains`, `Â +1 contains`, `K +3 starts`, `Y +1 starts`, `L +4 contains`, `R +6 contains`, `S +2 contains`, `T +6 starts/+5 contains`, `V +5 starts`, `B +2 contains`, and `D +1 contains`.
- Preserved the Batch 47 decisions that `cameră web` stays in `C`, `iac` stays in `I`, `webcam` and `yak` stay out unless explicitly reopened, no `Ă` or `Â` starts-with rows are forced, and rare buckets are not padded with obscure vocabulary.
- Completed Romanian Content Expansion Batch 47 slice coverage QA and next-prompt preparation on 2026-06-02.
- Recomputed canonical production totals from `content/ro/*.json`: `385` total records, `382` approved words, `382` approved ready images, `0` approved placeholders, and `3` rejected audit-trail rows: `ghicitoare`, `miez`, and `stinghie`.
- Confirmed new playable rare-letter routes are enabled and loaded: `K` -> `k`, `Q` -> `q`, `W` -> `w`, `X` -> `x`, and `Y` -> `y`.
- Confirmed `cameră web` stayed in `content/ro/words-c.json`, `iac` stayed in `content/ro/words-i.json`, and no production `webcam` or `yak` row exists.
- Recorded requested-target coverage after Batch 46 in the feature status, including `K` `2` starts / `1` contains / `3` mixed, `Q` `1` / `0` / `1`, `W` `1` / `2` / `3`, `X` `1` / `5` / `6`, `Y` `1` / `0` / `1`, `CE` `14` / `3` / `17`, `L` `14` / `78` / `92`, `R` `14` / `138` / `152`, `T` `11` / `81` / `92`, and `B` `23` / `21` / `44`.
- Batch 47 verification passed outside the sandbox after sandboxed `pnpm` attempts hung and then failed with `fetch failed`: `pnpm run validate:content`, `pnpm run lint`, `pnpm exec tsc --noEmit --incremental false`, `pnpm run check:gameplay`, and `pnpm run check:matching`; `git diff --check` passed after the Batch 47 status edits.
- Localhost route QA for representative new routes and images could not be completed because the expected existing dev server on `http://localhost:3000` was unavailable both inside and outside the sandbox; no alternate dev server or port was started.
- Recorded the next Romanian content task as a repeated Batch 44-style planning batch for a second production slice, with rare-letter stretch rows source-checked and no forced `Ă`, `Â`, `Q`, `W`, or `Y` padding.
- Completed Romanian Content Expansion Batch 46 production promotion and verification on 2026-06-02 after the human explicitly accepted all Batch 45 contact-sheet cells.
- Cropped all `15` accepted cells in fixed order from `assets/image-sources/ro/batch-45/contact-sheet.png`, normalized full-square backgrounds, checked thumbnail and circular-mask safety, post-processed through a refined `128 px` pixel-art grid with an `8`-level color snap, and promoted `256 x 256 px` lossless WebP assets below the `20 KB` hard maximum.
- Added production JSON rows and public assets for `kiwi`, `koala`, `xilofon`, `taxi`, `pix`, `boxă`, `mixer`, `walkie-talkie`, `cameră web`, `yo-yo`, `iac`, `quinoa`, `lampă`, `lună`, and `ceainic`.
- Added new enabled rare-letter manifests and routes for `K`, `Q`, `W`, `X`, and `Y`; updated `src/content/loaders.ts` and the gameplay compatibility fixture accordingly.
- Confirmed `cameră web` stayed in `content/ro/words-c.json` and `iac` stayed in `content/ro/words-i.json`; `webcam` and `yak` were not reintroduced.
- Confirmed post-Batch-46 production totals: `382` approved words, `382` ready images, `0` approved placeholders, and `3` rejected audit-trail rows.
- Verification passed outside the sandbox because sandboxed `pnpm` commands repeatedly hung and then failed with `fetch failed`: `pnpm run validate:content`, `pnpm run lint`, `pnpm exec tsc --noEmit --incremental false`, `pnpm run check:gameplay`, `pnpm run check:matching`, and `git diff --check`.
- `pnpm run validate:content` passed with existing image warning-threshold notices plus the new `ro-x-xilofon.webp` warning at `13,520 B`; it remains below the `20 KB` hard maximum.
- Localhost route QA for the new routes and images could not be completed because the expected existing dev server on `http://localhost:3000` was unavailable both inside and outside the sandbox; no alternate dev server or port was started.
- Recorded Batch 47 as the next Romanian content expansion step for slice coverage QA and the next prompt.
- Completed Romanian Content Expansion Batch 45 image briefs and contact sheet on 2026-06-02: added `docs/app-development-program/features/romanian-content-expansion/image-briefs-batch-45.md`.
- Generated one ordered, unlabeled Batch 45 review contact sheet at `assets/image-sources/ro/batch-45/contact-sheet.png` for `kiwi`, `koala`, `xilofon`, `taxi`, `pix`, `boxă`, `mixer`, `walkie-talkie`, `cameră web`, `yo-yo`, `iac`, `quinoa`, `lampă`, `lună`, and `ceainic`.
- Confirmed Batch 45 stayed review-only with no production word records, cropped cells, optimized public WebP assets, route/schema changes, placeholders, speech-target production metadata, admin editing, auth, database, billing, accounts, AI pronunciation, or clinical claims.
- Recorded the ready-to-copy Batch 46 prompt for human-accepted image promotion and full validation.
- Completed Romanian Content Expansion Batch 44 production-slice planning on 2026-06-02: added `docs/app-development-program/features/romanian-content-expansion/production-slice-planning-batch-44.md`.
- Verified borrowed spellings and Romanian display forms for the first slice, keeping `walkie-talkie`, `yo-yo`, `quinoa`, `kiwi`, `koala`, and `mixer`, revising `webcam` to `cameră web`, and revising `yak` to `iac`.
- Recorded Batch 44 canonical files, stable IDs, image paths, route/validation impacts, requested-target coverage impacts, and image brief directions for `kiwi`, `koala`, `xilofon`, `taxi`, `pix`, `boxă`, `mixer`, `walkie-talkie`, `cameră web`, `yo-yo`, `iac`, `quinoa`, `lampă`, `lună`, and `ceainic`.
...
- Recorded the ready-to-copy Batch 45 prompt for image briefs and one contact sheet, with a hard stop for human image review before production promotion.
- Completed Romanian Content Expansion Batch 41 planning on 2026-06-02: added `docs/app-development-program/features/romanian-content-expansion/requested-letter-and-sound-expansion-batch-41.md` and `docs/app-development-program/features/romanian-content-expansion/next-vocabulary-expansion-prompt-batch-41.md`.
- Completed Romanian Content Expansion Batch 42 candidate worksheet on 2026-06-02: added `docs/app-development-program/features/romanian-content-expansion/candidate-review-worksheet-batch-42-requested-letter-and-sound-expansion.md`.
- Completed Romanian Content Expansion Batch 43 human vocabulary review on 2026-06-02: added `docs/app-development-program/features/romanian-content-expansion/human-vocabulary-review-batch-43.md`.
- Recorded broad human approval of the Batch 42 candidate pool, including learnable stretch words and planning consideration for `webcam`, `walkie-talkie`, `yak`, `yo-yo`, `quinoa`, and `mixer`, while keeping borrowed spelling/source verification in Batch 44.
- Recorded the accepted Batch 43 decision that no production-ready starts-with `Ă` or `Â` candidates were found, and those buckets must not be forced with pronouns, deictics, nonstandard spellings, or obscure vocabulary.
...
- Confirmed Batch 42 stayed worksheet-only with no production word records, image assets, route/schema changes, placeholders, speech-target production metadata, admin editing, auth, database, billing, accounts, AI pronunciation, or clinical claims.
- Recomputed current requested-target coverage from `content/ro/*.json`; the Batch 41 coverage snapshot still matches the canonical manifests, with `367` approved words, `367` ready images, and `0` approved placeholders.
- Recorded Batch 42 source checks for Romanian rare-letter usage and Romanian speech/logopedie target-position and differentiation guidance.
- Recorded that Batch 43 must use the Batch 42 worksheet for human vocabulary review and first production-slice selection.
- Recorded the human decision that `Ă`, `Â`, `K`, `Q`, `W`, `X`, and `Y` should behave like normal playable letters once implemented, including starts-with, contains-only, mixed mode, setup, play, exclusion filtering, and admin inventory.
- Recorded the rare-letter and quantity policy for the next Romanian expansion: aim near `10` good additions per requested area where quality allows, use fewer for weak rare buckets, allow only a few concrete learnable stretch words where needed, avoid duplicate or near-synonym padding, allow imageable actions, and include but do not over-prioritize overlapping words.
- Recorded that Batch 42 must re-check Romanian speech/logopedie references before generating accepted candidates and must stay worksheet-only.
- Recorded the full requested-expansion batch sequence through Batch 47 and final QA, with a requirement that each later agent provide the prompt for the following batch.
- Corrected the Romanian G/GHI vocabulary issue on 2026-05-26: online dexonline/DOOM checks confirmed standard `chiuvetă` in the `C` / `CHI` pool (`https://dexonline.ro/intrare/chiuvet%C4%83/10192`) and `ghimbir` as a valid `GHI` replacement (`https://dexonline.ro/definitie/ghimbir`); the incorrect `ro-g-ghiuveta` / `ghiuvetă` record and unused public image were removed, `ro-g-ghimbir` / `ghimbir` was added as a concrete replacement G/GHI word, and `public/images/ro/g/ro-g-ghimbir.webp` was generated and optimized as a `256 x 256 px` lossless WebP asset below the warning threshold.
- Updated the content-dependent gameplay compatibility fixture because replacing `ghiuvetă` with `ghimbir` intentionally reduces the derived `V` contains-only pool from `15` to `14` while preserving the exact-size visible-subset countdown regression coverage.
...
Admin Words Inventory, Locale-Wide Content Refactor, Gameplay Inclusion Mode Selector, Wheel Setup UX Redesign, the mobile setup compression pass, Romanian Content Expansion Batches 40 through 69, and Wheel Setup Exclusion Filters Batch 4 are complete. The current Romanian production pack has `460` total records, `457` approved words, `457` ready images, `0` approved placeholders, and `3` rejected audit-trail rows. New playable routes for `K`, `Q`, `W`, `X`, and `Y` are enabled. Batch 66 promoted all accepted Batch 65 cells with the human-requested `tacâmuri` revision, Batch 67 completed slice coverage QA, Batch 68 planned the next review-only slice, and Batch 69 generated the contact sheet. The next approved Romanian content task is Batch 70: promote only human-accepted Batch 69 cells after review. Keep `cameră web` in `C`, keep `iac` in `I`, do not reintroduce `webcam` or `yak` without an explicit human decision, do not force `Ă` or `Â` starts-with rows, and do not pad rare buckets with obscure words.

Remaining release verification steps: stage and commit the production content/docs/assets, deploy or inspect the Vercel preview, verify deployed `/ro`, `/admin/words`, and representative `/ro/play/...` routes on mobile and desktop, and complete final human visual review of the deployed production image pack. Batch 40 corrective image guidance remains in `docs/image-pipeline.md`; future corrective image work still needs a new human-flagged list and must stop for review after contact-sheet generation.
