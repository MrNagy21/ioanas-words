...
Phase: Batch 8 implementation complete; admin words inventory Batch 4 complete; locale-wide content refactor complete; gameplay inclusion mode selector complete; Romanian content expansion Batch 14 complete

Overall status: Static Romanian starter content drives an interactive SVG wheel with segment labels, ready image rendering, stable placeholder fallbacks, spin selection, and spin animation. The wheel itself is the primary spin target, decorative frame effects stay fixed outside the rotating surface, word labels and the center letter stay upright, and spin animation uses a longer physical deceleration. Batch 4 adds the result modal, close/keep flow, client-side word removal, reset current letter, choose-another-letter paths, reduced-motion spin behavior, and a clear empty-wheel state after all current-letter words are removed. Batch 5 added local static content validation for Romanian letters, word manifests, image paths, duplicate IDs, letter buckets, and production status rules. Batch 6 selected pixel art, AI batch generation with human review, `256 x 256 px` lossless WebP, size targets, prompt/style guidelines, and word-ID naming conventions in `docs/image-pipeline.md`, then added local ready-image validation and a dry-run capable optimizer command. The `C`, `M`, `A`, and `P` starter image pilots now each have all 10 words ready. All four pilot sets are background-normalized, marked ready after visual review, and stored as local `256 x 256 px` WebP assets. The user manually verified the updated `M`, `A`, and `P` image batches in the running app and reported they work fine. Batch 8 removed the default word-preview list from the play screen, enlarged and simplified the wheel-first layout, improved the letter selection grid, added modal focus management and focus trapping, improved contrast for primary actions, and added a future-ready word-selection helper for later letter inclusion modes. A hydration warning on the play route was addressed by making SVG wheel numeric transforms deterministic and suppressing known browser-extension mutations on `<body>`. The one-word remaining wheel state draws a full SVG disk instead of a collapsed 360-degree arc, so the last word keeps its segment color. The next-phase development trajectory is now captured in `docs/app-development-program/`, including the recommended order: public `/admin/words` inventory, locale-wide content refactor, gameplay inclusion selector, Romanian content expansion, future practice-target architecture for Romanian letter groups, and review/QA workflow. Admin words inventory Batch 4 completed the public read-only `/admin/words` feature with polished dashboard spacing, clearer table and list readability, accessible repeated-link labels, explicit thumbnail/placeholder announcements, and feature acceptance documentation. A follow-up admin words update made each word row clickable and added a read-only details pop-up with a larger image, all canonical word fields, backdrop/Escape close behavior, and focus trapping. Locale-Wide Content Refactor is complete, with stable locale-wide helper APIs for manifests, approved words, exact Romanian derived pools, and image readiness counts. Gameplay Inclusion Mode Selector is complete, with `/ro/play/<letter>` using the mode-ready `GameplayContent` contract, defaulting to starts-with mode, opening a focused setup sheet for starts-with/contains-only/mixed mode and bounded word count, preserving cross-mode remove/reset behavior, and supporting result-modal random replacement from eligible off-wheel words. Post-QA UX polish moved mode/count controls out of the live play surface, removed `Altă literă` from the result modal, made every result modal action close the modal, and changed `Scoate` so removal shrinks the current wheel instead of automatically backfilling to the configured target count. The Romanian Content Expansion package now includes a completed Batch 1 coverage audit at `docs/app-development-program/features/romanian-content-expansion/coverage-audit-and-targets.md` and a completed Batch 2 candidate worksheet at `docs/app-development-program/features/romanian-content-expansion/candidate-review-worksheet-batch-2.md`. The audit confirmed then-current content had 40 approved words, 40 ready images, enabled `A/C/M/P` buckets only, broad alphabet gaps, and high-value speech-target gaps for `R`, `S`, `Z`, `Ș`, `J`, `Ț`, `CE/CI`, `GE/GI`, `F`, `V`, `L`, `T`, `D`, `C`, and `G`. The worksheet records 69 deduplicated candidate rows for the worksheet-first Tier 1 Speech Seed, with normalized forms, canonical starting letters, target positions, syllable complexity, clusters, contrast value, imageability, safety, decisions, and rejection/revision reasons. The pilot audit and worksheet phase is now complete; future Romanian expansion work is production preparation, and any implemented words/images must be production-ready once they enter manifests or public assets. Romanian Content Expansion Batch 3 confirmed the current production JSON shape can support accepted candidates without therapy metadata, kept speech-target metadata worksheet/status-only for this slice, tightened validation against unsupported fields and duplicate exact/folded word values, and added coverage-summary regression checks. Romanian Content Expansion Batch 4 added 15 accepted worksheet rows as production-bound draft metadata in canonical `B`, `G`, `J`, `R`, `S`, `U`, and `Z` word files, with disabled letter manifest entries and loader imports, while keeping all new records out of approved gameplay because no images were added and placeholder-backed approved words were not approved. Romanian Content Expansion Batch 5 added reviewed image briefs, generated and visually reviewed 15 child-safe pixel-art images, optimized canonical WebP assets under `public/images/ro/`, promoted all 15 target records to approved with ready app-owned images, enabled `B/G/J/R/S/U/Z`, and kept speech-target metadata out of production JSON. Romanian Content Expansion Batch 6 completed final QA for that first 15-word production slice: helper-derived `/admin/words` coverage now reported 55 approved Romanian words, 55 ready images, 0 placeholders, 11 enabled letters, 55 starts-with assignments, and 76 contains-only assignments. Romanian Content Expansion Batch 7 resolved the `Ș`/`Ț` content convention after human approval: exact bucket IDs, word files, and image folders stay Unicode, while word IDs and image filenames use ASCII prefixes `ro-sh-...` and `ro-tz-...`. A post-Batch 8 route fix now uses ASCII public play route segments `/ro/play/sh` and `/ro/play/tz`, resolving them internally to exact `ș` and `ț`. Romanian Content Expansion Batch 8 implemented the 17-word second production slice from accepted worksheet rows only: it added image briefs, reviewed pixel-art images, final canonical WebP assets, approved word records, and enabled `D/F/Ș/Ț` buckets while preserving exact `ș`/`ț` folders and using ASCII-safe routes and IDs. Helper-derived `/admin/words` coverage then reported 72 approved Romanian words, 72 ready images, 0 placeholders, 15 enabled letters, 72 starts-with assignments, and 120 contains-only assignments. Romanian Content Expansion Batch 9 planned the third production slice only, adding `third-production-slice-batch-9.md` with canonical files, IDs, image paths, categories, alt directions, duplicate checks, coverage impact, and readiness paths for `rochie`, `rachetă`, `iepure`, `scaun`, `nas`, `urs`, `coș`, `cămașă`, `dinți`, `cățel`, and `pijama`. Romanian Content Expansion Batch 10 implemented that 11-word third production slice with image briefs, reviewed pixel-art images, optimized canonical WebP assets, approved records, and new enabled `I` and `N` buckets. Helper-derived `/admin/words` coverage now reports 83 approved Romanian words, 83 ready images, 0 placeholders, 17 enabled letters, 83 starts-with assignments, and 186 contains-only assignments. Romanian Content Expansion Batch 11 reviewed that coverage, confirmed all 43 accepted Batch 2 worksheet rows are already implemented, selected a split vocabulary-first and image-second pipeline, and recorded the human-approved policy that approved placeholder-backed words may be visible in gameplay with a deliberately generic placeholder until final images exist. Romanian Content Expansion Batch 12 added `vocabulary-scope-spec-batch-12.md`, a researched broad by-letter scope with 126 new accepted placeholder-ready rows, documentation-only speech-target review fields, expected approved-word impact of 83 -> 209 if fully implemented, and the `Ă`/`Â`/`Î` ASCII convention: `a-breve`, `a-circ`, and `i-circ` route tokens with `ro-a-breve-`, `ro-a-circ-`, and `ro-i-circ-` ID/image prefixes. Romanian Content Expansion Batch 13 implemented all 126 accepted Batch 12 rows as approved placeholder-backed records, added enabled `E/H/Î/L/O/T/V` buckets, added the generic app-owned placeholder asset, wired the exact `Î` route `/ro/play/i-circ`, and now reports 209 approved words, 83 ready images, 126 placeholders, 24 enabled letters, 209 starts-with assignments, and 744 contains-only assignments. Romanian Content Expansion Batch 14 added `sound-coverage-scope-spec-batch-14.md`, a researched documentation-only scope with 110 accepted candidate rows for underfilled sound targets, explicit quality ceilings, and the separate practice-target model for bottom-row `CE/CI/GE/GI/CHE/CHI/GHE/GHI` options. If Batch 15 implements the full accepted scope, expected mixed coverage becomes `ș 36`, `j 14`, `ț 36`, `z 31`, `ce 14`, `ci 14`, `ge 10`, `gi 4`, `che 10`, `chi 9`, `ghe 8`, and `ghi 8`; approved words should move from 209 to 319, with final image replacement deferred to Batch 16.
...
Admin Words Inventory, Locale-Wide Content Refactor, Gameplay Inclusion Mode Selector, and Romanian Content Expansion Batch 14 are complete. The next Romanian content step is Batch 15: Sound Coverage Manifest Implementation And Start-Page Practice Targets. It should implement only accepted rows from `docs/app-development-program/features/romanian-content-expansion/sound-coverage-scope-spec-batch-14.md`, add a separate practice-target manifest/helper for `CE`, `CI`, `GE`, `GI`, `CHE`, `CHI`, `GHE`, and `GHI`, keep those sequence targets out of `content/ro/letters.json`, update `/ro/play/[letter]` target resolution and admin coverage, keep review-only speech-target metadata out of production JSON, and avoid final per-word images unless the scope is explicitly changed. Browser verification should target the always-running local dev server on port `3000`; if sandboxed `localhost:3000` checks fail, agents should retry outside the sandbox before treating the app as unavailable.
